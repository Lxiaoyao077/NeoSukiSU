...
NativeBridge(getAppProfile, jobject, jstring pkg, jint uid) {
	...
	jclass cls = GetEnvironment()->FindClass(env, "com/neosukisu/neosukisu/Natives$Profile");
...
NativeBridgeNP(getKernelPatchImplementation, jobject) {
	...
	jclass cls = GetEnvironment()->FindClass(env,
                                             "com/neosukisu/neosukisu/Natives$KernelPatchImplementation");
	if (cls == nullptr) {
	...
	jmethodID valuesMethod = GetEnvironment()->GetStaticMethodID(env, cls, "values",
                                                                 "()[Lcom/neosukisu/neosukisu/Natives$KernelPatchImplementation;");
...
NativeBridgeNP(getDynamicManager, jobject) {
	...
	jobject obj = CREATE_JAVA_OBJECT("com/neosukisu/neosukisu/Natives$DynamicManagerConfig");
	jclass cls = GetEnvironment()->FindClass(env, "com/neosukisu/neosukisu/Natives$DynamicManagerConfig");
...
NativeBridgeNP(getManagersList, jobject) {
    ...
    jobject obj = CREATE_JAVA_OBJECT("com/neosukisu/neosukisu/Natives$ManagersList");
    jclass managerListCls = GetEnvironment()->FindClass(env,
                                                        "com/neosukisu/neosukisu/Natives$ManagersList");
    ...
    if (cmd && count > 0) {
        for (int i = 0; i < count; i++) {
            jobject managerInfo = CREATE_JAVA_OBJECT_WITH_PARAMS(
                    "com/neosukisu/neosukisu/Natives$ManagerInfo",
                    "(II)V",
...
Java_com_neosukisu_neosukisu_magica_AppZygotePreload_forkDontCareAndExecKsud(JNIEnv *env,
                                                                           jclass clazz,
